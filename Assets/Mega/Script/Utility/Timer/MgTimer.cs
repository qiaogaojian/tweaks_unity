...
//	http://www.opsive.com
//
//	description:	MgTimer is a script extension for delaying (scheduling) methods
...
namespace Mega
{
    ...
    public class MgTimer : MonoBehaviour
    {
        ...
        /// <summary>
        /// returns false if this script was not added via the 'MgTimer.In'
        ...
        /// <summary>
        /// in Awake a check is made to see if the MgTimer component
        ...
        private void Update()
        {
            ...
            m_EventBatch = 0;
            while ((MgTimer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
            {
                ...
                if (m_EventIterator < 0)
                {
                    ...
                    m_EventIterator = MgTimer.m_Active.Count - 1;
                    break;
                }

                // prevent index out of bounds
                if (m_EventIterator > MgTimer.m_Active.Count - 1)
                    m_EventIterator = MgTimer.m_Active.Count - 1;

                // execute all due events
                if (Time.time >= MgTimer.m_Active[m_EventIterator].DueTime ||  // time is up
                    MgTimer.m_Active[m_EventIterator].Id == 0)                 // event has been canceled ('Execute' will kill it)
                    MgTimer.m_Active[m_EventIterator].Execute();
                else
                {
                    // handle pausing
                    if (MgTimer.m_Active[m_EventIterator].Paused)
                        MgTimer.m_Active[m_EventIterator].DueTime += Time.deltaTime;
                    else
                        // log lifetime
                        MgTimer.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
                }
        ...
        private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
        {
            ...
            {
               Debuger.LogError("Error: (MgTimer) Aborted event because function is null.");
                return;
            }
            ...
            if (m_MainObject == null)
            {
                ...
                if (!m_AppQuitting)
                {
                    ...
                    m_MainObject.AddComponent<MgTimer>();
            ...
            MgTimer.m_EventCount++;
            m_NewEvent.Id = MgTimer.m_EventCount;
            ...
            MgTimer.m_Active.Add(m_NewEvent);
        ...
        /// </summary>
        private static void Cancel(MgTimer.Handle handle)
        {
        ...
        public static void CancelAll()
        {
            ...
            for (int t = MgTimer.m_Active.Count - 1; t > -1; t--)
            {
                MgTimer.m_Active[t].Id = 0;
            }
        ...
        public static void CancelAll(string methodName)
        {
            ...
            for (int t = MgTimer.m_Active.Count - 1; t > -1; t--)
            {
                if (MgTimer.m_Active[t].MethodName == methodName)
                    MgTimer.m_Active[t].Id = 0;
            }
        ...
        {
            MgTimer.m_Active.Clear();
            MgTimer.m_Pool.Clear();
	...
	private void OnLevelWasLoaded()
#endif
        {
            ...
            for (int t = MgTimer.m_Active.Count - 1; t > -1; t--)
            {
                if (MgTimer.m_Active[t].CancelOnLoad)
                    MgTimer.m_Active[t].Id = 0;
            }
        ...
        /// <summary>
        /// provides the custom MgTimer editor class with Debuger info
        /// </summary>
        ...
        /// <summary>
        /// provides the custom MgTimer editor class with the name of a
        ...
        /// <summary>
        /// provides the custom MgTimer editor class with the id of a
        ...
        //					an event post initiation, schedule it with a 'MgTimer.Handle'
        //
        /////////////////////////////////////////////////////////////////////////////////
        public class Event
        {
            ...
            /// <summary>
            /// performs internal recycling of the MgTimer
            /// </summary>
            private void Recycle()
            {
                ...
                if (MgTimer.m_Active.Remove(this))
                    m_Pool.Add(this);
            ...
            private void Destroy()
            {
                ...
                MgTimer.m_Active.Remove(this);
                MgTimer.m_Pool.Remove(this);
        ...
        public class Handle
        {
            ...
            private MgTimer.Event m_Event = null;  // timer we're pointing at
            ...
            public int Id
            {
                ...
                set
                {
                    ...
                    for (int t = MgTimer.m_Active.Count - 1; t > -1; t--)
                    {
                        if (MgTimer.m_Active[t].Id == m_Id)
                        {
                            m_Event = MgTimer.m_Active[t];
                            break;
                        }
                    }
                    if (m_Event == null)
