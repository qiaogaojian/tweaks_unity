...
//	http://www.opsive.com
//
//	description:	MgFixedTimer is a script extension for delaying (scheduling) methods
...
//#define Debuger	// uncomment to display timers in the Unity Editor Hierarchy

using UnityEngine;
using System;
using System.Collections.Generic;

#if (UNITY_EDITOR && Debuger)
...
namespace Mega
{
    ...
    /// 强大的定时器工具
    /// </summary>
    public class MgFixedTimer : MonoBehaviour
    {
        ...
        /// <summary>
        /// returns false if this script was not added via the 'MgFixedTimer.In'
        ...
        /// <summary>
        /// in Awake a check is made to see if the MgFixedTimer component
        ...
        public static void SetMainGameObject(GameObject mainGo)
        {
            m_MainObject = mainGo;
        }
        ...
        private void FixedUpdate()
        {
            ...
            m_EventBatch = 0;
            while ((MgFixedTimer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
            {
                ...
                if (m_EventIterator < 0)
                {
                    ...
                    m_EventIterator = MgFixedTimer.m_Active.Count - 1;
                    break;
                }

                // prevent index out of bounds
                if (m_EventIterator > MgFixedTimer.m_Active.Count - 1)
                    m_EventIterator = MgFixedTimer.m_Active.Count - 1;

                // execute all due events
                if (Time.time >= MgFixedTimer.m_Active[m_EventIterator].DueTime ||  // time is up
                    MgFixedTimer.m_Active[m_EventIterator].Id == 0)                 // event has been canceled ('Execute' will kill it)
                    MgFixedTimer.m_Active[m_EventIterator].Execute();
                else
                {
                    // handle pausing
                    if (MgFixedTimer.m_Active[m_EventIterator].Paused)
                        MgFixedTimer.m_Active[m_EventIterator].DueTime += Time.deltaTime;
                    else
                        // log lifetime
                        MgFixedTimer.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
                }
        ...
        /// running the Update loop and drawing editor Debuger info)
        /// </summary>
        private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
        {
            ...
            {
                 Debuger.LogError("Error: (MgFixedTimer) Aborted event because function is null.");
                return;
            }
            ...
            if (m_MainObject == null)
            {
                ...
                if (!m_AppQuitting)
                {
                    ...
                    m_MainObject.AddComponent<MgFixedTimer>();
            ...
            MgFixedTimer.m_EventCount++;
            m_NewEvent.Id = MgFixedTimer.m_EventCount;
            ...
            MgFixedTimer.m_Active.Add(m_NewEvent);
            ...
            //如果timer handle被提供了，和event绑定之前，需要清空
            if (timerHandle != null)
            {
        ...
        /// </summary>
        private static void Cancel(MgFixedTimer.Handle handle)
        {
        ...
        public static void CancelAll()
        {
            ...
            for (int t = MgFixedTimer.m_Active.Count - 1; t > -1; t--)
            {
                MgFixedTimer.m_Active[t].Id = 0;
            }
        ...
        public static void CancelAll(string methodName)
        {
            ...
            for (int t = MgFixedTimer.m_Active.Count - 1; t > -1; t--)
            {
                if (MgFixedTimer.m_Active[t].MethodName == methodName)
                    MgFixedTimer.m_Active[t].Id = 0;
            }
        ...
        {
            MgFixedTimer.m_Active.Clear();
            MgFixedTimer.m_Pool.Clear();
	...
	private void OnLevelWasLoaded()
#endif
        {
            ...
            for (int t = MgFixedTimer.m_Active.Count - 1; t > -1; t--)
            {
                if (MgFixedTimer.m_Active[t].CancelOnLoad)
                    MgFixedTimer.m_Active[t].Id = 0;
            }
        ...
        /// <summary>
        /// provides the custom MgFixedTimer editor class with Debuger info
        /// </summary>
        ...
        /// <summary>
        /// provides the custom MgFixedTimer editor class with the name of a
        /// specific method in the Active list along with any arguments,
        /// plus a stack trace (if compiling with the Debuger define)
        /// </summary>
        ...
        /// <summary>
        /// provides the custom MgFixedTimer editor class with the id of a
        ...
        //
        //	MgFixedTimer.Event
        //  中文描述：Event是个内部类，因为Event可以循环使用，所以设计为不被外部引用，自己挥手。当Event初始化后，
        //   由MgFixedTimer.Handle定时调度
        ...
        //					an event post initiation, schedule it with a 'MgFixedTimer.Handle'
        //
        /////////////////////////////////////////////////////////////////////////////////
        public class Event
        {
            ...
            public bool CancelOnLoad = false;
            ...
            public void Execute()
            {
                ...
                {
                    Function();
                    //UnityEngine.Debuger.Log(Function.Method + ", " + (Function.Target ?? "none"));
                }
            ...
            /// <summary>
            /// performs internal recycling of the MgFixedTimer
            /// </summary>
            private void Recycle()
            {
                ...
                if (MgFixedTimer.m_Active.Remove(this))
                    m_Pool.Add(this);
            ...
            private void Destroy()
            {
                ...
                MgFixedTimer.m_Active.Remove(this);
                MgFixedTimer.m_Pool.Remove(this);
...
#if (UNITY_EDITOR && Debuger)
            /// <summary>
            /// used by the Debuger mode to fetch the callstack
            /// </summary>
        ...
        //					state of an event. the editor uses it to display Debuger info
        //
        /////////////////////////////////////////////////////////////////////////////////
        public class Handle
        {

            private MgFixedTimer.Event m_Event = null;  // timer we're pointing at
            ...
            public int Id
            {
                ...
                set
                {
                    ...
                    for (int t = MgFixedTimer.m_Active.Count - 1; t > -1; t--)
                    {
                        if (MgFixedTimer.m_Active[t].Id == m_Id)
                        {
                            m_Event = MgFixedTimer.m_Active[t];
                            break;
                        }
                    }
                    if (m_Event == null)
                         Debuger.LogError("Error: (" + this + ") Failed to assign event with Id '" + m_Id + "'.");
            ...
            public bool CancelOnLoad
            {
                ...
                set
                {
                    if (Active)
                    {
                     ...
                     Debuger.LogWarning("Warning: (" + this + ") Tried to set CancelOnLoad on inactive timer handle.");
                }
            }
