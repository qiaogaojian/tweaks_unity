...
// Please send feedback or bugreports to the.hedgehog.team@gmail.com
...
/// 	--------------
/// 		- C# migration
/// 		- Implementing delegate for sending messages. (Broadcast messages is rMegained with a parameter for javascript developpers)
...
public class EasyTouch : MonoBehaviour {
	...
	public delegate void SimplMegapHandler(Gesture gesture);
	public delegate void DoublMegapHandler(Gesture gesture);
	...
	public delegate void SimplMegap2FingersHandler(Gesture gesture);
	public delegate void DoublMegap2FingersHandler(Gesture gesture);
	...
	/// </summary>
	public static event SimplMegapHandler On_SimplMegap;
	/// <summary>
	/// Occurs when the number of taps is egal to 2 in a short time.
	/// </summary>
	public static event DoublMegapHandler On_DoublMegap;
	/// <summary>
	...
	/// <summary>
	/// Like On_SimplMegap but for a 2 fingers gesture.
	/// </summary>
	public static event SimplMegap2FingersHandler On_SimplMegap2Fingers;
	/// <summary>
	/// Like On_DoublMegap but for a 2 fingers gesture.
	/// </summary>
	public static event DoublMegap2FingersHandler On_DoublMegap2Fingers;
	/// <summary>
	...
	private enum EventName{ None,On_Cancel, On_Cancel2Fingers, On_TouchStart,On_TouchDown,On_TouchUp,On_SimplMegap,On_DoublMegap,On_LongTapStart,On_LongTap,
	On_LongTapEnd,On_DragStart,On_Drag,On_DragEnd,On_SwipeStart,On_Swipe,On_SwipeEnd,On_TouchStart2Fingers,On_TouchDown2Fingers,On_TouchUp2Fingers,On_SimplMegap2Fingers,
	On_DoublMegap2Fingers,On_LongTapStart2Fingers,On_LongTap2Fingers,On_LongTapEnd2Fingers,On_Twist,On_TwistEnd,On_PinchIn,On_PinchOut,On_PinchEnd,On_DragStart2Fingers,
				...
				case GestureType.Acquisition:
					if (FingerInTolerance(fingers[fingerIndex])){
						if (fingers[fingerIndex].tapCount<2){
							CreateGesture( fingerIndex, EventName.On_SimplMegap,fingers[fingerIndex], timeSinceStartAction, SwipeType.None,0,Vector2.zero);
						}
						else{
							CreateGesture( fingerIndex, EventName.On_DoublMegap,fingers[fingerIndex], timeSinceStartAction, SwipeType.None,0,Vector2.zero);
						}
		...
		switch (gesture){
			...
			case GestureType.Tap:
				...
				if (fingers[twoFinger0].tapCount<2 && fingers[twoFinger1].tapCount<2){
					CreateGesture2Finger(EventName.On_SimplMegap2Fingers,startPosition,position,deltaPosition,
					time, SwipeType.None,0,Vector2.zero,0,0,fingerDistance);				
				}
				else{
					CreateGesture2Finger(EventName.On_DoublMegap2Fingers,startPosition,position,deltaPosition,
					time, SwipeType.None,0,Vector2.zero,0,0,fingerDistance);
				}
			break;
		...
		switch(evnt){
			...
			case EventName.On_TouchUp:
				if (On_TouchUp!=null)
					On_TouchUp( gesture );
				break;
			case EventName.On_SimplMegap:
				if (On_SimplMegap!=null)
					On_SimplMegap( gesture);
				break;
			case EventName.On_DoublMegap:
				if (On_DoublMegap!=null)
					On_DoublMegap(gesture);
				break;
			...
			case EventName.On_TouchUp2Fingers:
				...
				break;
			case EventName.On_SimplMegap2Fingers:
				if (On_SimplMegap2Fingers!=null)
					On_SimplMegap2Fingers(gesture);
				break;
			case EventName.On_DoublMegap2Fingers:
				if (On_DoublMegap2Fingers!=null)
					On_DoublMegap2Fingers(gesture);
				break;
