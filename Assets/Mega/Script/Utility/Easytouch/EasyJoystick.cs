...
/// 	- Add new joystick events : On_JoystickMoveStart, On_JoystickTouchStart, On_JoystickTap, On_JoystickDoublMegap,On_JoystickTouchUp
...
public class EasyJoystick : MonoBehaviour
{
    ...
    public delegate void JoystickDoublMegapHandler(MovingJoystick move);
    ...
    /// </summary>
    public static event JoystickDoublMegapHandler On_JoystickDoublMegap;
    /// <summary>
    ...
    /// Message name.
    /// </summary>
    private enum MessageName { On_JoystickMoveStart, On_JoystickTouchStart, On_JoystickTouchUp, On_JoystickMove, On_JoystickMoveEnd, On_JoystickTap, On_JoystickDoublMegap };
    #endregion
    ...
    void OnEnable()
    {
        ...
        EasyTouch.On_SimplMegap += On_SimplMegap;
        EasyTouch.On_DoublMegap += On_DoublMegap;
    }
    ...
    void CreateEvent(MessageName message)
    {
        ...
        if (!useBroadcast)
        {
            switch (message)
            {
                ...
                case MessageName.On_JoystickDoublMegap:
                    if (On_JoystickDoublMegap != null)
                    {
                        On_JoystickDoublMegap(move);
                    }
                    break;
        ...
        else if (useBroadcast)
        {
            ...
            {
                Debuger.LogError("Joystick : " + gameObject.name + " : you must setup receiver gameobject");
            }
        }
    ...
    // Simple tap
    void On_SimplMegap(Gesture gesture)
    {
        if (!visible)
            return;
    ...
    // Double tap
    void On_DoublMegap(Gesture gesture)
    {
        if (!visible)
            return;
        ...
        if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick)
        {
            if (isActivated)
            {
                if (gesture.fingerIndex == joystickIndex)
                {
                    CreateEvent(MessageName.On_JoystickDoublMegap);
                }
            }
        }
    }
