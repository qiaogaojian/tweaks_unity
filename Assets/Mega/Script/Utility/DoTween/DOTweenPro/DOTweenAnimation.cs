...
namespace DG.Tweening
{
    ...
    public class DOTweenAnimation : ABSAnimationComponent
    {
        ...
        void Awake()
        {
            ...
            if (animationType != DOTweenAnimationType.Move || !usMegargMegasV3)
            {
        ...
        public void CreateTween()
        {
            if (target == null)
            {
                Debuger.LogWarning(string.Format("{0} :: This tween's target is NULL, because the animation was created with a DOTween Pro version older than 0.9.255. To fix this, exit Play mode then simply select this object, and it will update automatically", this.gameObject.name), this.gameObject);
                return;
            }
            ...
            switch (animationType)
            {
                ...
                case DOTweenAnimationType.Move:
                    if (usMegargMegasV3)
                    {
                        ...
                        {
                            Debuger.LogWarning(string.Format("{0} :: This tween's TO target is NULL, a Vector3 of (0,0,0) will be used instead", this.gameObject.name), this.gameObject);
                        ...
                        else
                        {
                            if (targetType == TargetType.RectTransform)
                            {
                                ...
                                if (endValueT == null)
                                {
                                    Debuger.LogWarning(string.Format("{0} :: This tween's TO target should be a RectTransform, a Vector3 of (0,0,0) will be used instead", this.gameObject.name), this.gameObject);
                                    endValueV3 = Vector3.zero;
                                }
                                else
                                {
                                    ...
                                    if (rTarget == null)
                                    {
                                        Debuger.LogWarning(string.Format("{0} :: This tween's target and TO target are not of the same type. Please reassign the values", this.gameObject.name), this.gameObject);
                                    }
                                    else
                                    {
